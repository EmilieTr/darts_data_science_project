...
def extract_results(text, pattern):
    data = {}
    ...
    for key, search_string in pattern.items():
        ...
        if start_index != -1:
            start_index += len(search_string)  # Start nach dem Suchmuster
            end_index = text.find("\n", start_index)  # Ende der Zeile suchen
            
            if end_index == -1:  
                end_index = len(text)  # Falls kein Zeilenumbruch, bis zum Ende nehmen
            
            data[key] = text[start_index:end_index].strip()
        else:
            data[key] = None  # Falls nicht gefunden, None setzen
        ...
        while facts:
            if facts.text.strip() == 'Fakten zur Person:':
                ...
                if fact:
                    ...
                    # Extract results6
                    data = data = extract_results(text, pattern)
