...
def extract_players(soup):
    ...
    return [player.find('a').get_text() for player in soup.find_all("div", class_="participant__participantName")]
...
def process_match_data(players, match_data):
    ...
    for score, coordinates in match_data:
        score_text, score_points = ("Lost Throw-in Home", score.split('VERLORENER ANWURF')[1]) if 'VERLORENER ANWURF' in score else ("", score)
        coordinate_list = [coord.replace("140+", "").replace("180", "") for coord in coordinates.split(", ")]
